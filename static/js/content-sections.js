(() => {
  ...
  const moveSlider = (track, direction) => {
    ...
    const wrappedPosition = nextPosition > maxScroll ? 0 : nextPosition < 0 ? maxScroll : nextPosition;
  ...
  const restartSlider = (track) => {
    ...
    sliderTimers.set(name, window.setInterval(() => moveSlider(track, 1), 6000));
  };
  document.querySelectorAll('[data-slider-action]').forEach((button) => button.addEventListener('click', () => {
    const track = document.querySelector(`[data-slider="${button.dataset.sliderTarget}"]`);
    moveSlider(track, button.dataset.sliderAction === 'next' ? 1 : -1);
    restartSlider(track);
  }));
  document.querySelectorAll('[data-slider]').forEach((track) => {
    restartSlider(track);
    track.addEventListener('mouseenter', () => window.clearInterval(sliderTimers.get(track.dataset.slider)));
    track.addEventListener('mouseleave', () => restartSlider(track));
    track.addEventListener('focusin', () => window.clearInterval(sliderTimers.get(track.dataset.slider)));
